...
func (cm *ContactManager) addKademlia2HelloResp(pMsg *Kademlia2HelloRespMsg) {
...
func (cm *ContactManager) addContactFromKademlia2Resp(pKadID *ID, ip uint32, udpPort uint16, version uint8) {
	pContact := cm.contactMap[ip]
...
func (cm *ContactManager) addKademlia2Resp(pMsg *Kademlia2RespMsg) {
	...
	for _, p := range pMsg.contacts {
		cm.addContactFromKademlia2Resp(p.pKadID, p.ip, p.updPort, p.version)
	}
}
