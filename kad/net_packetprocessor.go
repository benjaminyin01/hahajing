...
func (pp *PacketProcessor) processPacket(pPacket *Packet) {
	...
	case kademlia2HelloRes:
		pp.processKademlia2HelloResp(pPacket)
	case kademlia2Res:
		pp.processKademlia2Resp(pPacket)
	case kademlia2SearchRes:
		pp.processKademlia2SearchResp(pPacket)
	}
}

func (pp *PacketProcessor) processKademlia2HelloResp(pPacket *Packet) {
	msg := Kademlia2HelloRespMsg{}
	msg.set(pPacket)

	pp.pContactManager.addKademlia2HelloResp(&msg)
}

func (pp *PacketProcessor) processKademlia2Resp(pPacket *Packet) {
	msg := Kademlia2RespMsg{}
	if msg.set(pPacket) {
		if !pp.pSearchManager.addKademlia2Resp(&msg) {
			// It's not our response for search key
			pp.pContactManager.addKademlia2Resp(&msg)
		}
	}
}

func (pp *PacketProcessor) processKademlia2SearchResp(pPacket *Packet) {
	msg := Kademlia2SearchRespMsg{}
	if msg.set(pPacket) {
		pp.pSearchManager.addKademlia2SearchResp(&msg)
	}
}
