...
func (s *Socket) send(pPacket *Packet) {
	...
	//com.HhjLog.Infof("Socket: Send packet to %s:%d\n", iIP2Str(pPacket.ip), pPacket.port)
}
...
func (s *Socket) recv() {
	...
	buf, nReceiverVerifyKey, nSenderVerifyKey, _ := s.decrypt(buf[:n], remoteIP)
	if buf == nil {
	...
	/*
		if bEncrypt {
			com.HhjLog.Infof("Socket: Receive encrypt packet from %s:%d\n", iIP2Str(remoteIP), remotePort)
		} else {
			com.HhjLog.Infof("Socket: Receive non-encrypt packet from %s:%d\n", iIP2Str(remoteIP), remotePort)
		}
	*/
